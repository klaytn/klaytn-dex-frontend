...
const notEnough = computed(() => {
  return $kaikas.bigNumber(value.value).comparedTo(pool.value.balance) === 1
})
...
async function confirm() {
  try {
    ...
    const deposit = FarmingContract.methods.deposit(props.pool.id, $kaikas.utils.toWei(value.value))
